...
class WearableDataGenerator:
    ...
    def generate_user_data(
        ...
        mult = condition_multipliers.get(
            health_condition, condition_multipliers["normal"]
        )
        ...
        for date in dates:
            ...
            heart_rate = max(60, min(120, np.random.normal(base_hr * mult["hr"], 5)))
            ...
            sleep_hours = max(4, min(12, np.random.normal(7.5 * mult["sleep"], 1.5)))
            ...
            body_temp = max(36.0, min(38.5, np.random.normal(base_temp, 0.2)))

            # Stress level: 0-100 scale
            stress = min(
                100, max(0, np.random.normal(30 + (100 - heart_rate) * 0.5, 15))
            )
    ...
    def generate_federated_dataset(
        self, num_users: int = 100, num_nodes: int = 5, days_per_user: int = 30
        ...
        for node_id in range(num_nodes):
            ...
            for i in range(users_per_node):
                ...
                user_df = self.generate_user_data(
                    user_id=user_id, days=days_per_user, health_condition=condition
                )
