...
class MultiModalHealthRiskModel:
    ...
    def load_individual_models(self, models_dir):
        ...
        for air_quality_path in air_quality_paths:
            if air_quality_path.exists():
                self.air_quality_model = AirQualityHealthRiskModel.load(
                    str(air_quality_path)
                )
        ...
        if not air_quality_loaded:
            print(
                f"✗ Air quality model not found. Tried: {[str(p) for p in air_quality_paths]}"
            )
        ...
        if not weather_loaded:
            print(
                f"✗ Weather model not found. Tried: {[str(p) for p in weather_paths]}"
            )
    ...
    def _get_base_predictions(self, df_wearable, df_air_quality, df_weather):
        ...
        if (
            self.air_quality_model
            and df_air_quality is not None
            and len(df_air_quality) > 0
        ):
            try:
    ...
    def _normalize_risk_levels(self, predictions_dict):
        ...
        wearable_mapping = {"normal": "low", "at_risk": "moderate", "ill": "high"}
        ...
        weather_mapping = {"low": "low", "moderate": "moderate", "high": "high"}

        if "wearable" in predictions_dict and predictions_dict["wearable"] is not None:
            normalized["wearable"] = np.array(
                [
                    wearable_mapping.get(pred, "moderate")
                    for pred in predictions_dict["wearable"]
                ]
            )

        if (
            "air_quality" in predictions_dict
            and predictions_dict["air_quality"] is not None
        ):
            normalized["air_quality"] = np.array(
                [
                    air_quality_mapping.get(pred, "moderate")
                    for pred in predictions_dict["air_quality"]
                ]
            )
        ...
        if "weather" in predictions_dict and predictions_dict["weather"] is not None:
            normalized["weather"] = np.array(
                [
                    weather_mapping.get(pred, "moderate")
                    for pred in predictions_dict["weather"]
                ]
            )
        ...
        for i in range(num_samples):
            ...
            for key, preds in normalized_predictions.items():
                if preds is not None and i < len(preds):
                    ...
                    if (
                        key in probabilities_dict
                        and probabilities_dict[key] is not None
                    ):
    ...
    def predict_weighted(
        ...
        for i in range(num_samples):
            ...
            for key in ["wearable", "air_quality", "weather"]:
                if (
                    key in normalized_predictions
                    and normalized_predictions[key] is not None
                ):
                        ...
                        if (
                            ...
                            if hasattr(prob, "__iter__"):
                                ...
                                if len(prob) >= len(risk_levels):
                                    weighted_probs += (
                                        np.array(prob[: len(risk_levels)]) * weight
                                    )
                                    total_weight += weight
                                else:
    ...
    def predict(self, df_wearable=None, df_air_quality=None, df_weather=None, **kwargs):
        ...
        if self.strategy == "ensemble":
            ...
            return self.predict_weighted(
                df_wearable, df_air_quality, df_weather, weights
            )
        else:
    ...
    def evaluate(self, df_wearable, df_air_quality, df_weather, y_true):
        ...
        try:
            roc_auc = roc_auc_score(
                y_true_encoded, y_pred_proba, multi_class="ovr", average="weighted"
            )
        except:
            roc_auc = None
        ...
        metrics = {
            ...
            "classification_report": classification_report(
                y_true_encoded, y_pred_encoded, target_names=self.label_encoder.classes_
            ),
    ...
    def load(cls, filepath, models_dir=None):
        ...
        if models_dir and (
            instance.wearable_model is None
            or instance.air_quality_model is None
            or instance.weather_model is None
        ):
