...
class WeatherHealthRiskModel:
    ...
    def prepare_features(self, df):
        ...
        weather_features = ['temperature', 'humidity', 'pressure', 'wind_speed', 'visibility']
        weather_features = [col for col in weather_features if col in df_processed.columns]
        ...
        if 'temperature' in df_processed.columns and 'humidity' in df_processed.columns:
            ...
            temp_comfort = np.where(
                (df_processed['temperature'] >= 18) & (df_processed['temperature'] <= 24),
                1, 0
            )
    ...
    def predict(self, df):
        ...
        if hasattr(self.model, 'feature_names_in_') and self.model.feature_names_in_ is not None:
            ...
            if self.feature_names and len(X[0]) != len(expected_features):
                ...
                X_df = pd.DataFrame(X, columns=self.feature_names[:len(X[0])] if self.feature_names else None)
