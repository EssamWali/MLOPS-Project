...
class WearableHealthRiskModel:
    ...
    def __init__(self, model_type: str = 'random_forest', random_state: int = 42):
        ...
        self.feature_names = ['heart_rate', 'steps', 'sleep_hours', 'calories', 
                              'body_temperature', 'stress_level', 
                              'hr_rolling_mean', 'steps_rolling_mean', 
    ...
    def _get_model(self):
        ...
        if self.model_type == 'logistic_regression':
            ...
            return GradientBoostingClassifier(n_estimators=100, learning_rate=0.1, random_state=self.random_state)
    ...
    def feature_engineering(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df['hr_rolling_mean'] = df['heart_rate'].rolling(window=rolling_window).mean().fillna(df['heart_rate'])
        df['steps_rolling_mean'] = df['steps'].rolling(window=rolling_window).mean().fillna(df['steps'])
    ...
    def train(self, df: pd.DataFrame, target_col: str = 'health_condition', test_size: float = 0.2) -> Dict[str, float]:
        """
