...
class FederatedClient(fl.client.NumPyClient):
    ...
    def _load_data(self):
        ...
        print(f"Node {self.node_id}: Loaded {len(self.X_train)} training samples, "
    ...
    def get_parameters(self, config) -> List[np.ndarray]:
        ...
        if hasattr(self.model.model, 'coef_') and self.model_type == 'logistic_regression':
    ...
    def evaluate(self, parameters: List[np.ndarray], config: Dict) -> Tuple[float, int, Dict]:
        ...
        accuracy = accuracy_score(self.y_test, 
                                 self.label_encoder.transform(predictions))
...
def start_client(node_id: int, server_address: str = "localhost:8080",
                data_path: Optional[Path] = None, model_type: str = 'random_forest'):
    """
