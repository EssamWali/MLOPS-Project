...
try:
    df_wearable = pd.read_csv(os.path.join(DATA_DIR, "wearable_data.csv"), parse_dates=['timestamp'])
    df_air = pd.read_csv(os.path.join(DATA_DIR, "air_quality_data.csv"), parse_dates=['timestamp'])
    df_weather = pd.read_csv(os.path.join(DATA_DIR, "weather_data.csv"), parse_dates=['timestamp'])
    
    # Merge data for full-system analysis (simplified merge on timestamp/location)
    df_combined = df_wearable.merge(df_air, on='timestamp', how='left').merge(df_weather, on='timestamp', how='left')
    df_combined = df_combined.dropna(subset=['heart_rate', 'aqi']) # Drop rows if core data is missing
...
def generate_risk_scores(df: pd.DataFrame):
    ...
    risk_map = {'normal': 1, 'at_risk': 2, 'ill': 3, 'good': 1, 'moderate': 2, 'unhealthy': 3, 'very_unhealthy': 4, 'low': 1, 'high': 3}
    ...
    df['system_risk_score'] = df[['w_score', 'a_score', 't_score']].mean(axis=1).round(1)
...
health_authority_layout = html.Div([
    html.Div(className="row", children=[
        ...
        serve_kpi_card("Cities/Locations Monitored", f"{TOTAL_CITIES}", 'success'),
        serve_kpi_card("Average System Risk (1-4)", f"{df_full_analysis['system_risk_score'].mean():.2f}", 'warning'),
        serve_kpi_card("Max Risk Alerts", f"{len(df_full_analysis[df_full_analysis['system_risk_score'] >= 3])}", 'danger'),
    ]),
    
    html.Div(className="row", children=[
        html.Div(className="six columns chart-container", children=[
            dcc.Graph(
    ...
    # Time Series Plot
    html.Div(className="row chart-container", children=[
        dcc.Graph(
            id='time-series-trends',
            figure=px.line(
                df_full_analysis.set_index('timestamp').resample('D').mean().reset_index(),
                x='timestamp',
...
citizen_layout = html.Div([
    html.H4("Individual Citizen Health Overview (Simulated)", className="section-title"),
    
    # User Selection Dropdown
    dcc.Dropdown(
        id='user-selector',
        options=[{'label': f'User ID: {i}', 'value': i} for i in df_wearable['user_id'].unique()],
    ...
    html.Div(className="row chart-container", children=[
        dcc.Graph(id='individual-metrics-chart')
    ]),
])


app.layout = html.Div(children=[
    html.H1("Health Risk Prediction MLOps Dashboard", style={'textAlign': 'center'}),
    dcc.Tabs(id="tabs", value='tab-authority', children=[
...
def update_citizen_view(selected_user_id):
    ...
    latest_risk_level = "High" if latest_score >= 3 else ("Moderate" if latest_score >= 2 else "Low")
    latest_hr = df_user['heart_rate'].iloc[-1]
    latest_stress = df_user['stress_level'].iloc[-1]
    
    kpi_children = [
        serve_kpi_card("Current Risk Score", f"{latest_score:.1f}", 'danger' if latest_score >= 3 else 'warning'),
        serve_kpi_card("Overall Risk Level", latest_risk_level, 'danger' if latest_score >= 3 else 'warning'),
